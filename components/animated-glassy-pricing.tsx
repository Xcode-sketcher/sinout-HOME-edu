  ...
  useEffect(() => {
    ...
    const fragmentShaderSource = `
      ...
      void main(){
        ...
        float t = (sin(iTime * 0.5) + 1.0) * 0.5;
        vec3 foregroundColor = mix(orange, purple, t) * vec3(v.x * 0.5 + 0.5, v.y * 0.5 + 0.5, (.7 - v.y * v.x) * 0.5 + 0.5);
...
export const PricingCard = ({
  ...
  return (
    <div className={cardClasses.trim()}>
      {isPopular && (
        <div className="absolute -top-4 right-4 px-3 py-1 text-[12px] font-semibold rounded-full bg-primary text-primary-foreground dark:text-primary-foreground">
          Most Popular
        </div>
      )}
      <div className="mb-3">
    ...
    >
      ...
      <div className="relative z-10 w-full min-h-[70vh] flex flex-col items-center justify-center px-4 py-8">
        <div className="w-full max-w-7xl mx-auto text-center mb-14">
          <h1 className="text-[48px] md:text-[64px] font-extralight leading-tight tracking-[-0.03em] text-foreground font-display">
            {title}
          </h1>
